        ...
        <div class="row mt-5">
            <div class="col-sm-12">
                ...
                <table class="table table-striped" id="tableView">
                    ...
                    <tbody>
                    <!-- Select students and tutorial information form database-->
                        <?php
$sql = "SELECT * FROM `students`,`enrol`,`unit_second`,`tutorial`
                where `unit_second`.`unit_id`=`enrol`.`unit_id`
                and `students`.`studentid`=`enrol`.`student_id`
                and `tutorial`.`tutorial_id`=`enrol`.`tutorial_id`
                and `tutorial`.`tutor_id`='$session_userid'";
...
<script>
  ...
  $.get("checker_tut.php",{search_term:search_term}).done(function(data)
  {
      $("#output").html(data);
      $("#output").show();
      $("#searchPage").hide();
